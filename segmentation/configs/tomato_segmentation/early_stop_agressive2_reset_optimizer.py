...
custom_hooks = [
    dict(
        ...
        iters_per_stage=400,     # add samples every 200 iterations
        save_checkpoint=True,

        reset_optimizer_on_add=True,
        
        # Train loss decay parameters 
        window_size=50,          # window size for loss analysis
        decay_threshold=0.001,   # minimum decay rate
        patience=10,              # patience for slow decay
        
        # LR warmup после добавления сэмплов
        lr_warmup_after_add=False,
        lr_warmup_factor=3.0,
        lr_warmup_iters=50
    )
]
