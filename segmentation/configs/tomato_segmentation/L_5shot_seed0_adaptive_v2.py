# --------------------------------------------------------
# Adaptive Learning Config с Train Loss Decay Check
# По образцу mmdetection + адаптация к MMSegmentation
# --------------------------------------------------------
...
# УЛУЧШЕННЫЙ Hook с Train Loss Decay мониторингом (по образцу mmdetection)
custom_hooks = [
    dict(
        type='AdaptiveLearningWithLossDecayHook',
        iters_per_stage=200,     # Добавлять каждые 150 итераций
        save_checkpoint=True,
        
        # Train loss decay parameters 
        window_size=20,          # Окно для анализа loss
        decay_threshold=0.001,   # Минимальный decay rate
        patience=5,              # Терпимость к медленному decay
        
        # LR warmup после добавления сэмплов
        lr_warmup_after_add=False,
...
# Work directory
# work_dir = './work_dirs/adaptive_loss_decay_L_5shot_seed0'
